  ...
  if (data.length > 0) {
    ...
    rowsofUserTable = data.map((d) => {
      ...
      if (passbook == true) {
        singleDataRow.push(
          <td>
           <button type="button" style={{color:"white"}} class="btn btn-warning" onClick={()=>{handlePassbook(d)}}>View</button>
          </td>
        );
      }
